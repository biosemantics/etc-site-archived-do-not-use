...
import edu.arizona.biosemantics.etcsite.core.shared.model.FileType;
	...
	public TaskType getTaskType(int id) {
		...
		try(Query query = new Query("SELECT * FROM etcsite_tasktypes WHERE id = ?")) {
			...
			while(result.next()) {
				...
				taskType = new TaskType(id, edu.arizona.biosemantics.etcsite.core.shared.model.TaskTypeEnum.valueOf(name), inputFileType, inputDirectory, outputFileType, outputDirectory, created);
			}	
		}catch(Exception e) {
