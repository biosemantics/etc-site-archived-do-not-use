...
public class HighlightXMLWriter extends XMLWriter {
	...
	public void writeNode(Node node) throws IOException {
		for(int i=0; i<nodeList.getLength(); i++) {
			org.w3c.dom.Node highlightNode = nodeList.item(i);
	...
	private Node ifEqualsHiglight(Node node, org.w3c.dom.Node highlightNode) {
		if(node instanceof DefaultElement) {
			...
			for(int i=0; i<defaultElement.attributeCount(); i++) {
				...
				org.w3c.dom.Node attributeNode = highlightNode.getAttributes().item(i);
				attributesEqual &= attribute.getName().equals(attributeNode.getNodeName()) &&
						attribute.getValue().equals(attributeNode.getNodeValue());
			}
			
			if(attributesEqual && defaultElement.getText().equals(highlightNode.getTextContent()) && 
