...
public class AuthenticationService extends RemoteServiceServlet implements IAuthenticationService {
	...
	public RPCResult<AuthenticationResult> login(String email, String password) {
		try {
			User user = daoManager.getUserDAO().getUser(email, "none");
	...
	public RPCResult<AuthenticationResult> loginWithGoogle(String accessToken) {
		...
		try {
			...
			try(BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream()))) {
				...
				daoManager.getUserDAO().addUser(openIdProviderId, encryptedDummyPassword, firstName, lastName, "google");
				
				
				User user = daoManager.getUserDAO().getUser(openIdProviderId, "google");
				...
				daoManager.getUserDAO().updateUser(user.getId(), firstName, lastName, 
						user.getEmail(), encryptedDummyPassword, user.getAffiliation(), user.getBioportalUserId(), user.getBioportalAPIKey());
				user = daoManager.getUserDAO().getUser(openIdProviderId, "google"); //refresh the user after update.
	...
	public RPCResult<RegistrationResult> registerLocalAccount(int captchaId, String captchaSolution, String firstName, String lastName, String email, String password) {
		...
		try {
			...
			if (daoManager.getUserDAO().addUser(email, encryptedPassword, firstName, lastName, "none")) //registration was successful.
	...
	public RPCResult<UpdateUserResult> updateUser(AuthenticationToken authenticationToken, String oldPassword, String firstName, String lastName, String email, String newPassword, String affiliation, String bioportalUserId, String bioportalAPIKey) {
		...
		try{ 
			...
			if (daoManager.getUserDAO().updateUser(userId, firstName, lastName, email, encryptedPassword, affiliation, bioportalUserId, bioportalAPIKey)){
				user = daoManager.getUserDAO().getUser(userId);
	...
	public RPCResult<PasswordResetResult> requestPasswordResetCode(int captchaId, String captchaSolution, String openIdProviderId) {
		...
		try {
			User user = daoManager.getUserDAO().getUser(openIdProviderId, "none");
			if (user == null)
			...
			PasswordResetRequest oldRequest = daoManager.getPasswordResetRequestDAO().getRequest(userId);
			if (oldRequest != null){
				...
				daoManager.getPasswordResetRequestDAO().removeRequests(userId);
			}
			...
			daoManager.getPasswordResetRequestDAO().addRequest(userId, authenticationCode);
	...
	public RPCResult<PasswordResetResult> requestPasswordReset(String openIdProviderId, String code, String newPassword){
		...
		try{
			...
			PasswordResetRequest request = daoManager.getPasswordResetRequestDAO().getRequest(userId);
			if (request == null)
			...
			daoManager.getUserDAO().updateUser(
					user.getId(), 
					user.getFirstName(), 
					user.getLastName(), 
					user.getEmail(), 
	...
	@Override
	public RPCResult<RequestCaptchaResult> createCaptcha() {
		int id = daoManager.getCaptchaDAO().insert();
