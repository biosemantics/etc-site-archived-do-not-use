...
public class TermsInOrderCategoryDAO {
	...
	public void addTermsInOrderCategory(int categoryId, String termName) throws SQLException, ClassNotFoundException, IOException {
		...
		try (Query query = new Query("SELECT termID FROM `terms_in_order_category` WHERE categoryId = ? AND termName = ?", "otolite")) {
			query.setParameter(1, categoryId);
			query.setParameter(2, termName);
			ResultSet resultSet = query.execute();
			if(!resultSet.next()) {
				try(Query insertQuery = new Query("INSERT INTO `terms_in_order_category` (`categoryID`, `termName`) VALUES (?, ?)", "otolite")) {
